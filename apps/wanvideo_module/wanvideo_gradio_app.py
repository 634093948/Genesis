...
project_root = current_dir.parent.parent  # Repository root
...
# Import core components directly from current repo
from compat import comfy_stub
from core import folder_paths_ext
...
class WanVideoWorkflow:
    ...
    def generate_video(
        ...
        enable_dram_optimization: bool,
        auto_hardware_tuning: bool,
        ...
        # Sampler Advanced
        use_tf32: bool,
        use_cublas_gemm: bool,
        force_contiguous_tensors: bool,
        ...
        try:
            ...
            # Step 10: Decode to video with tiling to reduce VRAM
            video_result = self.decoder.decode(
                vae=vae,
                samples=samples,
                enable_vae_tiling=True,  # Enable tiling to reduce VRAM
                tile_x=272,  # Match ComfyUI workflow settings
                tile_y=272,
                tile_stride_x=144,
                tile_stride_y=128
            )
...
def create_interface():
    ...
    def generate_with_progress(*args):
        ...
        (positive_prompt, negative_prompt, width, height, num_frames,
         ...
         base_precision, load_device, rms_norm_function,
         lora_enabled, lora_name, lora_strength,
         compile_enabled, compile_backend, block_swap_enabled, blocks_to_swap,
         enable_cuda_optimization, enable_dram_optimization, auto_hardware_tuning,
         vram_threshold, num_cuda_streams, bandwidth_target,
         offload_txt_emb, offload_img_emb, vace_blocks_to_swap,
         use_tf32, use_cublas_gemm, force_contiguous_tensors, fuse_qkv_projections,
         debug_mode,
        ...
        video_path, video_array, metadata = workflow.generate_video(
            ...
            vace_blocks_to_swap=vace_blocks_to_swap,
            use_tf32=use_tf32,
            use_cublas_gemm=use_cublas_gemm,
            force_contiguous_tensors=force_contiguous_tensors,
    ...
    with gr.Blocks(title="WanVideo Genesis", theme=gr.themes.Soft()) as interface:
        ...
        with gr.Tabs():
            ...
            with gr.Tab("Optimization"):
                ...
                with gr.Row():
                    with gr.Column():
                        gr.Markdown("### VAE Decode Optimization")
                        gr.Markdown("**Note**: VAE tiling is now enabled by default to reduce VRAM usage during decode.")
                        gr.Markdown("Tiling parameters are optimized based on ComfyUI workflow (272x272 tiles with 144x128 stride).")
                        gr.Markdown("This significantly reduces decode VRAM from ~20GB to ~8GB for 1280x720 videos.")
